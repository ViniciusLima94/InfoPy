...
def ulamData_ksg(count):
	...
	MI12 =  KSGestimatorMI(data[:, 0], data[:, 1], k = 6, norm = False, noiseLevel = 1e-8)
	MI21 =  KSGestimatorMI(data[:, 1], data[:, 0], k = 6, norm = False, noiseLevel = 1e-8)
	TE12 =  KSGestimatorTE(data[:, 0], data[:, 1], k = 6, norm = False, noiseLevel = 1e-8) 
	TE21 =  KSGestimatorTE(data[:, 1], data[:, 0], k = 6, norm = False, noiseLevel = 1e-8) 
	return np.array([MI12, MI21, TE12, TE21])

def ulamData_kernel(count):
	print('Coupling = ' + str(count))
	data = np.loadtxt('data_maps/ulam/ulam_'+str(count)+'.dat', delimiter=',')
	MI12 =  KernelEstimatorMI(data[:, 0], data[:, 1], kernel = 'gaussian', bw = 0.3, norm=False, delay = 1)
	MI21 =  KernelEstimatorMI(data[:, 1], data[:, 0], kernel = 'gaussian', bw = 0.3, norm=False, delay = 1)
	TE12 =  KernelEstimatorTE(data[:, 0], data[:, 1], kernel = 'gaussian', bw = 0.3, norm=False) 
	TE21 =  KernelEstimatorTE(data[:, 1], data[:, 0], kernel = 'gaussian', bw = 0.3, norm=False) 
	return np.array([MI12, MI21, TE12, TE21])
